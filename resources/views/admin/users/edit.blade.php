...
@section('content')
    <div class="max-w-3xl">
        ...
        <h1 class="mt-2 text-xl font-semibold text-slate-800">Ubah User</h1>

        <form action="{{ route('admin.users.update', $user) }}" method="POST" enctype="multipart/form-data" class="mt-6 space-y-5 rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
            @csrf
            @method('PUT')
            ...
            <div class="grid gap-5 md:grid-cols-2">
                ...
                <div>
                    <label for="password" class="block text-sm font-medium text-slate-600">Reset Kata Sandi</label>
                ...
                <div>
                    ...
                    @if($user->avatar_path)
                        <img src="{{ $user->avatar_url }}" alt="Avatar" class="mt-2 h-14 w-14 rounded-full border border-slate-200 object-cover">
                    @endif
                </div>
            </div>
            ...
            <div>
                ...
                <p class="text-xs text-slate-500">Centang menu/fitur yang dapat diakses user ini. Kosongkan untuk menggunakan akses bawaan sesuai role.</p>
                <div class="mt-3 grid gap-3 sm:grid-cols-2">
                    @php
                        $selectedModules = (array) old('modules', $user->modules->pluck('key')->all());
                    @endphp
                    @foreach($modules as $module)
                        <label class="flex items-start gap-3 rounded-xl border border-slate-200 bg-slate-50 px-4 py-3">
                            <input type="checkbox" name="modules[]" value="{{ $module->key }}" @checked(in_array($module->key, $selectedModules, true))
